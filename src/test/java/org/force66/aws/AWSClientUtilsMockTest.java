...
/**
 * This test ensures that AWSClientUtils can satisfy one of its use cases and be mocked.
 * The point is *not* to test Mockito's ability to "mock"; that's their job. The point is 
 * to make sure we haven't done anything to AWSClientUtils that *prevents* it from being mocked.
 * @author D. Ashmore
 *
 */
...
public class AWSClientUtilsMockTest {
	...
	public void setUp() throws Exception {
		...
		Mockito.when(awsClientUtils.buildAmazonS3(ArgumentMatchers.anyString(), ArgumentMatchers.any(AssumeRoleRequest.class)))
			.thenReturn(amazonS3Mock);
	}
