...
public class AWSClientUtilsTest {
  ...
  public void setup() throws Exception {
    ...
    Mockito.when(amazonS3ClientBuilderProxyMock.withRegion(ArgumentMatchers.any(Regions.class)))
        .thenReturn(amazonS3ClientBuilderProxyMock);
    Mockito
        .when(amazonS3ClientBuilderProxyMock
            .withCredentials(ArgumentMatchers.any(AWSCredentialsProvider.class)))
        .thenReturn(amazonS3ClientBuilderProxyMock);
    ...
    Mockito
        .when(awsSecurityTokenServiceMock.assumeRole(ArgumentMatchers.any(AssumeRoleRequest.class)))
  ...
  public void testCreateAssumedRoleCredentials() throws Exception {
    ...
    Assert.assertEquals(TEST_SESSIONTOKEN,
        ((BasicSessionCredentials) credProvider.getCredentials()).getSessionToken());
  }
  ...
  public void testCreateAssumedRoleCredentials_AssumedRoleException() throws Exception {
    RuntimeException rExpt = new RuntimeException("crap");
    Mockito.when(awsSecurityTokenServiceMock.assumeRole(assumeRoleRequest)).thenThrow(rExpt);
  ...
  private void validateRole() {
    ArgumentCaptor<AWSCredentialsProvider> credsArg =
